...
var $author$project$Main$init = F3(
	function (_v0, url, key) {
		...
		var _v1 = function () {
			if ((route.$ === 'Exercise') && (route.a === 1)) {
				return _Utils_Tuple2(
					$elm$core$Maybe$Just(1),
					$elm$core$Maybe$Just($author$project$Pages$Ex01$init));
			} else {
		...
		var initModel = _v1.b;
		return _Utils_Tuple2(
			{currentExercise: currentEx, ex01Model: initModel, key: key},
			...
			case 'UrlChanged':
				...
				if ((route.$ === 'Exercise') && (route.a === 1)) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{currentExercise: $elm$core$Maybe$Nothing, ex01Model: $elm$core$Maybe$Nothing}),
						$elm$core$Platform$Cmd$none);
				}
			default:
				var subMsg = msg.a;
				var _v3 = model.ex01Model;
				if (_v3.$ === 'Just') {
					var ex01Model = _v3.a;
					var _v4 = A2($author$project$Pages$Ex01$update, subMsg, ex01Model);
					var newModel = _v4.a;
					var cmd = _v4.b;
					return _Utils_Tuple2(
						_Utils_update(
...
var $author$project$Main$exerciseLink = function (n) {
	var paddedNumber = A3(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$href('/ex' + paddedNumber)
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('Exercise ' + paddedNumber)
					]))
			]));
};
...
var $author$project$Main$leftPane = A2(
	...
	_List_fromArray(
		[
			...
			A2(
				$elm$core$List$map,
				$author$project$Main$exerciseLink,
				A2($elm$core$List$range, 1, 57)))
		]));
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				if (_v0.$ === 'Just') {
					if (_v0.a === 1) {
						var _v1 = model.ex01Model;
						if (_v1.$ === 'Just') {
							var ex01Model = _v1.a;
							return A2(
								$elm$html$Html$map,
								$author$project$Main$Ex01Msg,
								$author$project$Pages$Ex01$view(ex01Model));
						} else {
							return $elm$html$Html$text('Loading Ex01...');
						}
					} else {
						var n = _v0.a;
